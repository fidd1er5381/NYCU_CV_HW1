...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, 
        ...
        if use_se:
            self.se = SEModule(planes * self.expansion, reduction=se_reduction)
...
class ResNetRS(nn.Module):
    def __init__(self, block, layers, num_classes=1000, use_se=True, se_reduction=32):
        super(ResNetRS, self).__init__()
        self.inplanes = 64
        self.use_se = use_se
        self.se_reduction = se_reduction
        
        self.conv1 = nn.Conv2d(3, self.inplanes, kernel_size=9, stride=2, padding=4, bias=False)
...
def resnetrs50(num_classes=1000, **kwargs):
    model = ResNetRS(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, 
...
def plot_confusion_matrix(model, loader, classes, normalize=False, cmap='Blues', tick_every=10):
    model.eval()
    all_preds = []
    all_labels = []
...
def get_transform(is_train=True):
    if is_train:
        return transforms.Compose([
            ...
            transforms.RandAugment(num_ops=2, magnitude=15), # 隨機增強
            transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3),
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
            transforms.RandomErasing(p=0.25)  # 隨機擦除
        ])
    else:
        return transforms.Compose([
            transforms.Resize(293),  # 224 * (256/224) = 256, 256 * (293/256) ~= 293
